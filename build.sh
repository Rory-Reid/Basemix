...
dotnet publish src/Basemix.UI/Basemix.UI.csproj -c Release -f net6.0-android --no-restore -p:AndroidKeyStore=True -p:AndroidSigningKeyPass="${ANDROID_SIGNING_KEY}" -p:AndroidSigningStorePass="${ANDROID_SIGNING_KEY}" -p:AndroidSigningKeyStore="${ANDROID_KEY_STORE}"
