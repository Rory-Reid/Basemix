using Basemix.Lib.Media;
using Shouldly;

namespace Basemix.Tests;

public class ImageProcessorTests
{
    // Minimal valid 1x1 red JPEG (from a known-good source)
    private static readonly byte[] MinimalJpeg =
    [
        
    ];

    [Fact]
    public void ResizeImage_detects_jpeg_format()
    {
        using var stream = new MemoryStream(MinimalJpeg);
        var result = ImageProcessor.ResizeImage(stream, 400);
        result.Format.ShouldBe("jpeg");
    }

    [Fact]
    public void ResizeImage_detects_png_format()
    {
        // Create a minimal 1x1 PNG by encoding via SkiaSharp
        using var bitmap = new SkiaSharp.SKBitmap(1, 1, SkiaSharp.SKColorType.Rgb888x, SkiaSharp.SKAlphaType.Opaque);
        bitmap.SetPixel(0, 0, SkiaSharp.SKColors.Red);
        using var skImage = SkiaSharp.SKImage.FromBitmap(bitmap);
        using var pngData = skImage.Encode(SkiaSharp.SKEncodedImageFormat.Png, 100);
        var pngBytes = pngData.ToArray();

        // Verify the bytes actually start with PNG magic
        pngBytes[0].ShouldBe((byte)0x89);
        pngBytes[1].ShouldBe((byte)0x50); // P

        using var stream = new MemoryStream(pngBytes);
        var result = ImageProcessor.ResizeImage(stream, 400);
        result.Format.ShouldBe("png");
    }
}
