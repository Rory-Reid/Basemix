...
public class OwnerStatsTests : StatsSqliteIntegration
{
    private readonly Faker faker = new();
    private readonly StatsSqliteFixture fixture;
    private readonly SqliteStatisticsRepository repository;

    public OwnerStatsTests(StatsSqliteFixture fixture) : base(fixture)
    {
    ...
    public async Task Returns_top_owners_from_bred_litters()
    {
        ...
        if (topOwners.Count > 1)
        {
            ...
            statistics.Owners.ShouldSatisfyAllConditions(
                stats => stats.TopOwners.Count.ShouldBe(3),
    ...
    [Fact]
    public async Task Doesnt_count_unbred_rats_in_top_owners()
    {
        ...
        foreach (var rat in notBredRatsWithOwner.Take(this.faker.Random.Int(1, notBredRatsWithOwner.Count))) // Seed some in an unbred litter, some not connected
        {
